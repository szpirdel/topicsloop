"""
Django management command to import 200 articles with AI-powered categorization
Imports 100 arXiv abstracts + 100 PLOS ONE articles

Usage:
    python manage.py import_articles_full
    python manage.py import_articles_full --dry-run  # Preview without saving
"""
...
class Command(BaseCommand):
    help = 'Import 200 articles (100 arXiv abstracts + 100 PLOS ONE articles) with AI categorization'
    ...
    def handle(self, *args, **options):
        ...
        # Define queries for different domains (100 articles from arXiv)
        # Target: ~5-6 articles per query
        arxiv_queries = [
            # Computer Science (30 articles)
            {'query': 'cat:cs.AI artificial intelligence', 'expected_domain': 'Computer Science > AI'},
            {'query': 'cat:cs.LG machine learning', 'expected_domain': 'Computer Science > Machine Learning'},
            {'query': 'cat:cs.CV computer vision', 'expected_domain': 'Computer Science > Computer Vision'},
            {'query': 'cat:cs.NE neural networks', 'expected_domain': 'Computer Science > Neural Networks'},
            {'query': 'cat:cs.RO robotics', 'expected_domain': 'Computer Science > Robotics'},
            {'query': 'cat:cs.CL natural language processing', 'expected_domain': 'Computer Science > NLP'},

            # Life Sciences (25 articles)
            {'query': 'cat:q-bio.NC neuroscience brain', 'expected_domain': 'Life Sciences > Neuroscience'},
            {'query': 'cat:q-bio.GN genetics genomics', 'expected_domain': 'Life Sciences > Genetics'},
            {'query': 'cat:q-bio.PE evolution biodiversity', 'expected_domain': 'Life Sciences > Evolution'},
            {'query': 'cat:q-bio.CB cell biology', 'expected_domain': 'Life Sciences > Cell Biology'},
            {'query': 'cat:q-bio.BM molecular biology', 'expected_domain': 'Life Sciences > Molecular Biology'},

            # Physics & Engineering (25 articles)
            {'query': 'cat:physics.med-ph medical physics', 'expected_domain': 'Physical Sciences > Medical Physics'},
            {'query': 'cat:physics.app-ph applied physics', 'expected_domain': 'Physical Sciences > Applied Physics'},
            {'query': 'cat:astro-ph astrophysics cosmology', 'expected_domain': 'Physical Sciences > Astrophysics'},
            {'query': 'cat:cond-mat materials science', 'expected_domain': 'Physical Sciences > Materials Science'},
            {'query': 'renewable energy solar wind', 'expected_domain': 'Engineering > Renewable Energy'},

            # Mathematics & Statistics (10 articles)
            {'query': 'cat:math.ST statistics probability', 'expected_domain': 'Mathematics > Statistics'},
            {'query': 'cat:math.OC optimization', 'expected_domain': 'Mathematics > Optimization'},

            # Environmental Sciences (10 articles)
            {'query': 'climate change modeling', 'expected_domain': 'Environmental Sciences > Climate Change'},
            {'query': 'environmental sustainability ecology', 'expected_domain': 'Environmental Sciences'},
        ]

        # PLOS ONE queries (100 articles)
        # Target: ~5 articles per query
        plos_queries = [
            # Life Sciences & Medicine (40 articles)
            {'query': 'neuroscience AND memory', 'expected_domain': 'Life Sciences > Neuroscience'},
            {'query': 'genetics AND gene expression', 'expected_domain': 'Life Sciences > Genetics'},
            {'query': 'immunology AND immune response', 'expected_domain': 'Life Sciences > Immunology'},
            {'query': 'microbiology AND bacteria', 'expected_domain': 'Life Sciences > Microbiology'},
            {'query': 'cancer AND tumor', 'expected_domain': 'Medicine > Oncology'},
            {'query': 'cardiovascular AND heart disease', 'expected_domain': 'Medicine > Cardiology'},
            {'query': 'diabetes AND metabolic', 'expected_domain': 'Medicine > Endocrinology'},
            {'query': 'mental health AND depression', 'expected_domain': 'Medicine > Psychiatry'},

            # Environmental Sciences (20 articles)
            {'query': 'climate change AND ecosystem', 'expected_domain': 'Environmental Sciences > Climate Change'},
            {'query': 'biodiversity AND conservation', 'expected_domain': 'Environmental Sciences > Conservation'},
            {'query': 'pollution AND environmental impact', 'expected_domain': 'Environmental Sciences > Pollution'},
            {'query': 'sustainable development', 'expected_domain': 'Environmental Sciences > Sustainability'},

            # Social Sciences (15 articles)
            {'query': 'psychology AND behavior', 'expected_domain': 'Social Sciences > Psychology'},
            {'query': 'education AND learning', 'expected_domain': 'Social Sciences > Education'},
            {'query': 'economics AND inequality', 'expected_domain': 'Social Sciences > Economics'},

            # Sports & Health (10 articles)
            {'query': 'sports AND exercise performance', 'expected_domain': 'Sports & Athletics'},
            {'query': 'nutrition AND dietary intervention', 'expected_domain': 'Health & Wellness > Nutrition'},

            # Technology & Engineering (15 articles)
            {'query': 'artificial intelligence AND application', 'expected_domain': 'Computer Science > AI'},
            {'query': 'renewable energy AND solar', 'expected_domain': 'Engineering > Renewable Energy'},
            {'query': 'materials science AND nanotechnology', 'expected_domain': 'Physical Sciences > Materials Science'},
        ]

        self.stdout.write('📥 Starting full article import (200 articles: arXiv + PLOS ONE)...\n')

        # Import from arXiv (100 articles - abstracts)
        self.stdout.write(self.style.SUCCESS('🔬 Importing from arXiv (100 short abstracts)...'))
        arxiv_articles = importer.import_from_arxiv(arxiv_queries, max_results_per_query=6)
        self.stdout.write(f'  Found {len(arxiv_articles)} articles from arXiv\n')

        # Import from PLOS ONE (100 articles - full texts)
        self.stdout.write(self.style.SUCCESS('📚 Importing from PLOS ONE (100 articles)...'))
        plos_articles = importer.import_from_plos(plos_queries, max_results_per_query=5)
